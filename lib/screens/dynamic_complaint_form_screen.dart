...
import 'dart:io';
import '../models/dynamic_complaint_model.dart';
...
class DynamicComplaintFormScreen extends StatefulWidget {
  const DynamicComplaintFormScreen({super.key});

  @override
  State<DynamicComplaintFormScreen> createState() => _DynamicComplaintFormScreenState();
}

class _DynamicComplaintFormScreenState extends State<DynamicComplaintFormScreen> {
  ...
  List<DynamicCrimeType> _availableCrimeTypes = [];
  DynamicCrimeType? _selectedCrimeType;
  ...
  Future<void> _loadCrimeTypes() async {
    ...
    try {
      ...
      final dynamicCrimeTypes = crimeTypesWithUnits
          .map((ctw) => DynamicCrimeType.fromCrimeTypeWithUnit(ctw))
          .toList();
  ...
  void _onCrimeTypeSelected(DynamicCrimeType? crimeType) {
    setState(() {
  ...
  Future<void> _submitComplaint() async {
    ...
    try {
      ...
      final complaint = DynamicComplaint.create(
      ...
      final complaintId = await _submitDynamicComplaint(complaint);
  ...
  Future<String?> _submitDynamicComplaint(DynamicComplaint complaint) async {
    try {
              ...
              _buildSectionCard(
                ...
                children: [
                  if (_isLoadingCrimeTypes)
                    ...
                    const SizedBox(height: 8),
                    DropdownButtonFormField<DynamicCrimeType>(
